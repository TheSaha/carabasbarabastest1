...
$key1 = 'PIDARASIII';
$key2 = 'MOSKVAP';
//require 'Templates/SchoolboyJebany.php';
...
if (isset($_COOKIE['hash'])) {
	...
	if($WebsiteOffline == 1) {
		if($user['rank'] != 100 and $user['rank'] != 19) {
...
//SETTINGS
 ...
 // if ($user['airgold'] < 0 || $user['DropCount'] < 0){
	 // $db->exec('UPDATE `users` SET `isBanned` = 1 WHERE `steamid` = '.$user['steamid']);
	 // $db->exec('UPDATE `users` SET `ReasonBan` = "Cheating with AirDrop!" WHERE `steamid` = '.$user['steamid']);
 // }
// // $sql1337 = $db->query('SELECT * FROM `users` WHERE `needToDeposit` = 1000 AND `user` = '.$db->quote($user['steamid']));
// // if($user['needToDeposit'] == 1000  and $user['tDeposits'] == 0) {
				// // $db->exec('UPDATE `users` SET `needToDeposit` = 3000 WHERE `steamid` = '.$user['steamid']);
			// //}
			
			// if($user['DropCount'] >= 12)
			// {
				// $db->exec('UPDATE `users` SET `DropCount` = 11 WHERE `steamid` = '.$user['steamid']);
			// }
			// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				  ...
				  // if($user['DropCount'] == 1) {
				// $numLOLEC = 1;
			// }else if($user['DropCount'] == 2){
				// $numLOLEC = 2;
			// }else if($user['DropCount'] == 0){
				// $numLOLEC = 0;
			// }else if($user['DropCount'] == 3){
				// $numLOLEC = 3;
			// }else if($user['DropCount'] == 4){
				// $numLOLEC = 4;
			// }else if($user['DropCount'] == 5){
				// $numLOLEC = 5;
			// }else if($user['DropCount'] == 6){
				// $numLOLEC = 6;
			// }else if($user['DropCount'] == 7){
				// $numLOLEC = 7;
			// }else if($user['DropCount'] == 8){
				// $numLOLEC = 8;
			// }else if($user['DropCount'] == 9){
				// $numLOLEC = 9;
			// }else if($user['DropCount'] == 10){
				// $numLOLEC = 10;
			// }else if($user['DropCount'] == 11){
				// $numLOLEC = 11;
			// }else if($user['DropCount'] == 12){
				// $numLOLEC = 12;
			// }else if($user['DropCount'] == 13){
				// $numLOLEC = 13;
			// }else if($user['DropCount'] == 14){
				// $numLOLEC = 14;
			// }else if($user['DropCount'] == 15){
				// $numLOLEC = 15;
			// }else if($user['DropCount'] == 16){
				// $numLOLEC = 16;
			// }
...
$mdtwithdraw = 50000000;

//DEPOSIT SETTINGS:
//$sc1 = 'ASDAFAF1Q24234324DWFSDFSSS';
$sc1 = 'MJD5KFLL6H8O4HD799GHDJJN81JVJ8H8F39RJ3JF93MFOIWJH9FJW9J0JK092J9ODFJNIFNH2EWIFUI33HBE78IUFHIE';
//WITHDRAW SETTINGS
//$sc2 = 'ADADASD132R1RWFSDFDFSDFSDF';
$sc2 = 'GNVDJIOFUI99UT40KJ9JEOJF9EJF09EU0F3J0FJ30FJ30JFG90EUJGF0ERIF0EUI0GI0GI093JG93UGOUBFGE7GFE7HF2NND2ND199390UR30'.$slot;
//PORT OF BOT:
$portBot = 6849;
$Zange = $db->query('SELECT `balance` FROM `users` WHERE `user` = '.$db->quote($user['steamid']));
...
switch ($_GET['page']) {
	...
	case 'deposit':
		...
		break;
			...
			case 'play':
		$page = getTemplate('play.php', array('user'=>$user));
		echo $page;
		break;
	...
	//case 'redeemgroup':
		...
		//$out = curl('https://api.steampowered.com/ISteamUser/GetUserGroupList/v1/?key=86E5E4B19BBCDDC2D2D40B7B966A54D3&steamID='.$user['steamid'].'&format=json');
		...
		//}
		//if(!$isingroup) exit(json_encode(array('success'=>false, 'error'=>'You are not in PUBGROLL group.')));
		//if($isingroup) {
	...
	case 'redeem':
		...
		$out = curl('https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=86E5E4B19BBCDDC2D2D40B7B966A54D3&steamid='.$user['steamid'].'&format=json');
		$out = json_decode($out, true);
		
		if(!$out['response']) exit(json_encode(array('success'=>false, 'error'=>'Your profile is private')));
		$csgo = false;
		foreach ($out['response']['games'] as $key => $value) {
			if($value['appid'] == 578080) $csgo = true;
		}
		if(!$csgo) exit(json_encode(array('success'=>false, 'error'=>'You dont have PUBG.')));
		...
		try
		{
			...
			if (!$openid->mode) {
				...
				if ($openid->validate()) {
					...
					$url = "https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=86E5E4B19BBCDDC2D2D40B7B966A54D3&steamids=$matches[1]";
					$json_object = file_get_contents($url);
					$json_decoded = json_decode($json_object);
					foreach ($json_decoded->response->players as $player) {
						$steamid = $player->steamid;
						$name111 = $player->personaname;
						$name = substr($name111, 0, 16);
	...
	if(!$user) exit(json_encode(array('success'=>false, 'error'=>'You must login to access the deposit.')));
		...
		$inv = curl('https://steamcommunity.com/profiles/'.$user['steamid'].'/inventory/json/578080/2/');
	...
	//exit(json_encode(array('success' => false, 'error' => $key)));
		case 'deposit_js':
		if(!$user) exit(json_encode(array('success'=>false, 'error'=>'You must login to access the deposit.')));
		if($_COOKIE['tid']) {
        ...
        foreach ($inv['rgInventory'] as $key => $value) {
            ...
            for ($i = 0; $i < count(assetids); $i++) {
                if($kek != 0) break;
                if ((preg_match('/(Sticker|Souvenir)/', $name) and $assetids[$i] == $value['id']) || ($prices['response']['items'][$name]['value']*10 < 99999 and $assetids[$i] == $value['id'])) {
                    $kek++;
					//exit(json_encode(array('success' => false, 'error' => $price1)));
                }
            }
        }
		$price1 = $prices['response']['items'][$name]['value'];
        if ($kek != 0) exit(json_encode(array('success' => false, 'error' => '11')));
		...
		// $rooo = ['summa'];
		// $lolec = filter_var($_GET['checksum']);
		// if ($lolec < 1000 and $assetids[$i] == $value['id']){
		// exit(json_encode(array('success' => false, 'error' => $rooo)));
		// }
	...
	case 'withdraw_js':
		...
		if(!$sum) {
		...
		} elseif($user['WithdrawBanned'] != 0 && $user['steamid'] != '') {
			$Message = '';
			...
			$out = array('success'=>false,'error'=>'Error: You were banned to withdraw from 82.146.41.244 by Administrator. Reason: '.$Message);
		} else {
			reset($items);
			$bot = key($items);
			...
			if($out['success'] == false) {
				...
				$db->exec('UPDATE `users` SET `needToDeposit`=`needToDeposit`+0 WHERE `steamid`='.$db->quote($user['steamid']));
				//$int333 = 5;
				$int333 = $user['needToDeposit'];
				$intt = $int333*1000;
				$needTodeposit = intval($intt);

				$db->exec('UPDATE `users` SET `available`=`available`-'.$db->quote($sum).' WHERE `steamid`='.$db->quote($user['steamid']));

				$db->exec('UPDATE `users` SET `needToDeposit`=`needToDeposit` WHERE `steamid`='.$db->quote($user['steamid']));
...
function getUserSteamAvatar($steamid){
    $link = file_get_contents('https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=86E5E4B19BBCDDC2D2D40B7B966A54D3&steamids='.$steamid.'&format=json');
...
<div id="AirDropInfo" onClick="window.location.href='#'">
      <div id="wnd2">
        <h2>What is AirDrop?</h2><br>
		
		<h5>	  AirDrop is the ability to regularly get free coins!<br>  	 When opened gives a random number of coins.<br>
   To get the AirDrop you need to play roulette.  	 <br>Place your bets, get AirDrop, open it and get free coins! </h5>
      </div>
    </div>
