...
var LANG = 1;
var IGNORE = [];
var sounds_rolling = new Audio('http://82.146.41.244/template/sounds/rolling.wav');
var sounds_tone = new Audio('http://82.146.41.244/template/sounds/tone.wav');

var sounds_rolling2 = new Audio('http://82.146.41.244/template/sounds/rolling.wav');
var sounds_tone2 = new Audio('http://82.146.41.244/template/sounds/tone.wav');

var sounds_type = new Audio('http://82.146.41.244/template/sounds/safebox/write.mp3');
var sounds_won = new Audio('http://82.146.41.244/template/sounds/safebox/win.mp3');

var sounds_boom = new Audio('http://82.146.41.244/template/sounds/mines/boom.mp3');
var sounds_click = new Audio('http://82.146.41.244/template/sounds/mines/click.mp3');
...
function onMessage(msg) {
	...
	if (m.type == "roll") {
		...
		//set choice p1:
		$('#P1choice').attr('src', 'http://82.146.41.244/template/img/flipcoin/'+choiceplayer1+'-icon.png');
		//set avatar p2:
		$('#P2avatar').attr('src', avatarplayer2);
		//set name p2:
		$('#P2name').text(numeplayer2);
		//set choice p2:
		$('#P2choice').attr('src', 'http://82.146.41.244/template/img/flipcoin/'+choiceplayer2+'-icon.png');
		...
		if(watchingGame == 1 && watchingGameID != 0) {
			...
			$('#P1name').text(NameP1);
			//set choice p1:
			$('#P1choice').attr('src', 'http://82.146.41.244/template/img/flipcoin/'+PickP1+'-icon.png');
			//set avatar p2:
			...
			//set choice p2:
			if(PickP1 == 'ct') {
				$('#P2choice').attr('src', 'http://82.146.41.244/template/img/flipcoin/t-icon.png');
			}else if(PickP1 == 't') {
				$('#P2choice').attr('src', 'http://82.146.41.244/template/img/flipcoin/ct-icon.png');
			}
...
function emotes(str) {
	...
	for (var i = 0; i < a.length; i++) {
		str = str.replace(new RegExp(a[i] + "( |$)", "g"), "<img src='http://82.146.41.244/template/img/site/" + a[i] + ".png'> ");
	}
	return str;
}
...
function chat(x, msg, name, icon, steamid, rank, lang, hide, verify) {
	...
	if (lang == LANG || x == "italic" || x == "error" || x == "alert" || x == "system" || x == "systemm") {
		...
		msg = emotes(msg);
		var toChat = "";
		if (x == "italic") toChat = "<div class='chat-msg'><img class='chat-img rounded' data-steamid='SYSTEM' data-name='System' src='http://82.146.41.244/favicon.ico'><div><span>System</span></div> <div>" + msg + "</div></div>";
        else if (x == "error") toChat = "<div class='chat-msg'><img class='chat-img rounded' data-steamid='SYSTEM' data-name='System' src='http://82.146.41.244/favicon.ico'><div><span>Error</span></div> <div class='text-danger'>" + msg + "</div></div>";
        else if (x == "alert") toChat = "<div class='chat-msg'><img class='chat-img rounded' data-steamid='SYSTEM' data-name='System' src='http://82.146.41.244/favicon.ico'><div><span>System</span></div> <div class='text-success'>" + msg + "</div></div>";
        else if (x == "system") toChat = "<div class='text-success' style='margin-left: 33px'>" + msg + "</div></div>";
        else if (x == "systemm") toChat = "<div class='chat-msg'><img class='chat-img rounded' data-steamid='SYSTEM' data-name='System' src='http://82.146.41.244/favicon.ico'><div><span>System</span></div>";
        else if (x == "player") {
...
$(document).ready(function() {
	...
	function cfUpdateHistory() {
	   $("#cfHist").load("http://82.146.41.244/getFlips.php");
	...
	function crUpdateHistory() {
	   $("#crHist").load("http://82.146.41.244/getCRHistory.php");
	   var refreshId = setInterval(function() {
	      $("#crHist").load('http://82.146.41.244/getCRHistory.php');
	   }, 1000);
	   $.ajaxSetup({ cache: false });
	...
	function sbUpdateHistory() {
	   $("#sbHist").load("http://82.146.41.244/getSB.php");
	   var refreshId = setInterval(function() {
	      $("#sbHist").load('http://82.146.41.244/getSB.php');
	   }, 1000);
	   $.ajaxSetup({ cache: false });
...
function changeLang(id) {
    ...
    if(LANG == '1') {
    	...
    	$('#changeLang0').attr('src', 'http://82.146.41.244/template/img/lang/en.png');
    }else if(LANG == '2') {
    	langName = 'Português';
    	$('.userson').html('Usuários Online: <span id="isonline"></span>');
    	$('.lang-select').text('Chat');
    	$('#changeLang0').attr('src', 'http://82.146.41.244/template/img/lang/br.png');
    }else if(LANG == '3') {
    	langName = 'Русский';
    	$('.userson').html('oнлайн: <span id="isonline"></span>');
    	$('.lang-select').text('Чат');
    	$('#changeLang0').attr('src', 'http://82.146.41.244/template/img/lang/ru.png');
    }
