...
switch ($_GET['page']) {
		...
		try
		{
			...
			if (!$openid->mode) {
				...
				if ($openid->validate()) {
					...
					$url = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=86E5E4B19BBCDDC2D2D40B7B966A54D3&steamids=$matches[1]";
	...
	if(!$user) exit(json_encode(array('success'=>false, 'error'=>'You must login to access the confirm.')));
		...
		if(($out['success'] == true) && ($out['action'] == 'accept') && ($row['status'] != 1)) {
			...
			if($row['status'] != '2') {
				$db->exec('UPDATE `users` SET `tDeposits` = `tDeposits` WHERE `steamid` = '.$db->quote($user['steamid']));
