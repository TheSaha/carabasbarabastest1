...
// if (connection.escape(user.DropSum) == 0){

						// amountWon = 45;
					
// }else if(connection.escape(user.DropSum) < 	300){
// if(randomCase == 1) {
						// amountWon = 15;
					...
					// }else if(randomCase == 7) {
						// amountWon = 20;
					// }
// }else if(connection.escape(user.DropSum) > 	299){
// if(randomCase == 1) {
						// amountWon = 1;
					// }else if(randomCase == 2) {
						// amountWon = 2;
...
// var AirBet3 = getRandomInt(1, 100);
// var airbet3 = AirBet3;
// logger.debug(airbet3);
...
// connection.query('SELECT `DropCount`, `steamid`, `balance` FROM `users` WHERE `user` = '+connection.escape(user.steamid), function(err, row) {
			// if(err) {
				// logger.debug("err");
				// }
				// if(row) {
				...
				// }
// });
connection.query("SELECT `DropCount`, `balance` FROM `users` WHERE `steamid` = "+connection.escape(user.steamid), function(err, rows, fields){
    if(err) return console.log(err);
	// connection.query('UPDATE `users` SET `DropCount` = `DropCount` + 1 WHERE `steamid` = '+pool.escape(user.steamid), function(err, rows, fields){
    // if(err) return console.log(err);
	// });
    query('UPDATE `users` SET `DropCount` = `DropCount` + 1 WHERE `steamid` = '+pool.escape(user.steamid));
	logger.debug(connection.escape(user.DropCount) + " AZAZA " + connection.escape(user.steamid) + " LOLEC " + connection.escape(user.balance));

});


//if (connection.escape(user.DropCount) < 5){
	if (1==1){
...
// connection.connect();
// if (connection.escape(user.DropCount) < 5){
// query('UPDATE `users` SET `DropCount` = `DropCount` + 1 WHERE `steamid` = '+connection.escape(user.steamid));
