...
var IntervalDeBetuireBoti = IntervalulLaBoti*1100;
var BotsStarted = 0;
...
io.on('connection', function(socket) {
	...
	socket.on('hash', function(hash) {
		query('SELECT `balance`,`avatar`,`needToDeposit`,`rank`,`name`,`steamid`, `openedbox`, `DropCount`, `freecase`, `case24` FROM `users` WHERE `hash` = '+pool.escape(hash), function(err, row) {
			if((err) || (!row.length)) {
				...
				var prome = time() - row[0].freecase;
				var trouble = 28801-prome;
				if (trouble <= 0){
				trouble = 0}
				
				var prometei = time() - row[0].case24;
				var trouble24 = 86400-prometei;
				if (trouble24 <= 0){
				trouble24 = 0}
				...
				socket.emit('message', {
					...
					free24:trouble24,
					freec: trouble,
					DropC: row[0].DropCount,
					infff: row[0].needToDeposit,
					Jcount: JackpotTimer-Jwait,
					icon: row[0].avatar,
					maxbet: maxbet,
					minbet: minbet,
...
function addChatBots() {
	...
	var Messages = ["Hello, dudes! All in gold. LOL:D", "fuck this website, I lost all!", "omfggggg...", "that was almost dudeeee", "no way, I am giving up!", "Hello? I will win.", "ezzz", "idinahuij", "kurwa site dah", "omgggg", "scam site reported  i lost allz my coins", "omfgg, who is the owner. iwant items back...", "I can predict all!!", "Red now!!!", "Blackkkkk", "black 9/2", "izi pizi gold", "nooooooo", "fuckkk my life", "Moderator ban me pls, I hate this website", "omfggg, are you idiot!?!","what the fuck|?!!", "ggez I am god!!"
	...
	, " <i> Message deleted. </i>  "
	, " LOL duude  "
	, "Keepo Keepo Keepo  KappaPride omg rly?  "
	...
	, "lets go on holidays :D "
	, "Keepo   "
	, "<i> Message deleted. </i> ", " FailFish  Keepo   ", "FailFish FailFish  Keepo   ", " <i> Message deleted. </i> "
	, "AGAIN ((( "
	...
	, " black now FailFish FailFish FailFish "
	, " BAD LUCK FailFish "
	, " BAD FUCKING LUCK "
	, " ezzz money) "
	, " lool win ezzzz "
...
function pickBot(botID){
	if(botID == 1) {
		...
		BotBetName = "csgo500.com cs.money";
...
function AirDrop1(m, user, socket) {
			...
			query('SELECT `DropCount`,`DropSum`, `openedbox` FROM `users` WHERE `steamid`='+pool.escape(user.steamid), function(err, row) {
				if(err) return;
				var SumaLuata = m.amount;
...
if (vasya.DropSum == 0){amountWon=90;
}else if(vasya.DropSum == 90) {amountWon=65;
}else if(vasya.DropSum == 155) {amountWon=55;
}else if(vasya.DropSum > 155 && vasya.DropSum < 380) {
...
}else if(vasya.DropSum >= 380 && vasya.DropSum < 520) {
		var randomsoso = getRandomInt(1, 7);
		random22 = randomsoso;
		if (random22 == 1){
			amountWon = 20;
	...
	}else if(vasya.DropSum >= 520 && vasya.DropSum < 720) {
		var randomsoso = getRandomInt(1, 7);
		random22 = randomsoso;
		if (random22 == 1){
			amountWon = 25;
	...
	logger.debug('[OpenAir] User: '+pool.escape(user.steamid)+'/__DropSum: '+vasya.DropSum + '.__VsegoDropov: ' +vsegoDrop + '.__Won: ' + amountWon);
	  socket.emit('message', {
						type: 'alert',
					...
					//////////////////////////////////////////////
// query('SELECT `DropCount`, `openedbox` FROM `users` WHERE `steamid` = '+pool.escape(user.steamid), function(err, row) {
		// if((err) || (!row.length)) {
...
// if (vasya.DropSum == 0){amountWon = 60;
// var DropSSS = vasya.DropSum;
// }else if(DropSSS == 60) {amountWon = 30;
// }else if(DropSSS == 90) {amountWon = 25;
// }else if(DropSSS > 90 && vasya.DropSum < 280) {
	// amountWon = 15;
// }else{
	// amountWon = 5;
// }
...
// var aga;
// if (vasya.DropSum == 0){amountWon=313;
// var DropSSS = vasya.DropSum;
// }else if(DropSSS == 60) {amountWon=323;
// }else if(DropSSS == 90) {amountWon=333;
// }else if(DropSSS > 90 && vasya.DropSum < 280) {
	// amountWon=343;
// }else{
	// amountWon=353;
// }
	// logger.debug(vasya.DropSum + ':' +vsegoDrop + ':' + amountwon);
	// var randomsoso = getRandomInt(1, 7);
		// random22 = randomsoso;
		// if (random22 == 1){
			// amountWon = 25;
...
// }else if(vasya.DropSum >= 280 && vasya.DropSum < 420) {
...
// }else if(vasya.DropSum >= 420 && vasya.DropSum < 600) {
...
// }}
  ...
  // logger.debug(pool.escape(user.steamid) + ".___Viigrish:_" + amountWon + ".___VsegoDROPOV:_"+vsegoDrop);
  // socket.emit('message', {
						// type: 'alert',
						// alert: 'You opened AirDrop. You won '+amountWon+' coins]'
					// });
...
/////////////////////////////////////////////////////////////
					...
					setTimeout(function() {
						query('UPDATE `users` SET `balance`=`balance`+'+amountWon+' WHERE `steamid`='+pool.escape(user.steamid));
						query('UPDATE `users` SET `DropSum`=`DropSum`+'+amountWon+' WHERE `steamid`='+pool.escape(user.steamid));
						//query('SELECT `DropSum` FROM `users` WHERE `hash` = '+pool.escape(hash));
						// logger.debug(pool.escape(user.DropSum));
						// logger.debug(pool.escape(user.balance));
						if(amountWon >= SumaLuata) {
					...
					//AEZAKMI logger.debug('[AIR-DROP] User: '+pool.escape(user.steamid)+' | Tip: 1 | Numar: *');
...
function caseOpen(m, user, socket) {
	if(m.tipBox) {
		if(m.tipBox == "1") {
			...
			//query('SELECT `summa` FROM `trades` WHERE `user`='+pool.escape(user.steamid)+' AND `status`=1 AND `time` >= '+pool.escape(time())+'-86400', function(err, row) {
query('SELECT `needToDeposit` FROM `users` WHERE `steamid`='+pool.escape(user.steamid), function(err, row) {
		if(err) return;
		//logger.debug('odmgldfmgkmd');
		logger.debug(row[0].needToDeposit);
		//row[0].needToDeposit === undefined ||
		if (row[0].needToDeposit !=0){
			socket.emit('message', {
						type: 'error',
						enable: true,
						error: 'You need Deposit '+row[0].needToDeposit+' coins!'
					});
			//return;
			}else{
			...
			//query('UPDATE `users` SET `freecase` = '+pool.escape(time())+' WHERE `steamid`='+pool.escape(user.steamid));
			query('SELECT `balance`, `freecase` FROM `users` WHERE `steamid`='+pool.escape(user.steamid), function(err, row) {
				if(err) return;
				var SumaLuata = m.amount;
				var kamaz = pool.escape(time());
			//logger.debug(kamaz);
			// logger.debug(kamaz-1000);
				var raznica = kamaz - row[0].freecase;
				//logger.debug(raznica);
				if(raznica<28800){
					var eshoo = 28801-raznica;
				socket.emit('message', {
						type: 'error',
						enable: true,
						error: 'Wait more time'+eshoo
					});
...
//OLD
			...
			//query('UPDATE `users` SET `freecase` = '+pool.escape(time())+' WHERE `steamid`='+pool.escape(user.steamid));
			query('SELECT `balance`, `case24` FROM `users` WHERE `steamid`='+pool.escape(user.steamid), function(err, row) {
				if(err) return;
				var SumaLuata = m.amount;
				var kamaz = pool.escape(time());
			//logger.debug(kamaz);
			// logger.debug(kamaz-1000);
				var raznica = kamaz - row[0].case24;
				//logger.debug(raznica);
				if(raznica<86400){
					var eshoo = 86400-raznica;
				socket.emit('message', {
						type: 'error',
						enable: true,
						error: 'Wait more time'+eshoo
				...
				if(row[0].balance < SumaLuata) {
					...
					setTimeout(function() {
						query('UPDATE `users` SET `case24` = '+pool.escape(time())+' WHERE `steamid`='+pool.escape(user.steamid));
					}, 750);
...
//query('UPDATE `freecase` = '+pool.escape(time())+'  FROM `users` WHERE `steamid`='+pool.escape(user.steamid));
...
function AABB(){
var KARLASON = getRandomInRange(3000,20000);;
}
...
// vsego = vasya.DropCount + vasya.openedbox;
  ...
  if (m.amount<100){///////////////////////////////////////////////////////////////
	  var random11 = getRandomInt(1, 20);
		random111 = random11;
		if (random111 == 2)
		{
	  victory = 1;
		} 
  }else if (m.amount>=100 && m.amount<500){ ////////////////////////////////////////////////////////
   var random11 = getRandomInt(1, 15);
		random111 = random11;
		if (random111 == 2)
		{
	  victory = 1;
		}
  }else if (m.amount>=500 && m.amount<1000){ ////////////////////////////////////////////////////////
   var random11 = getRandomInt(1, 12);
		random111 = random11;
		if (random111 == 2)
		{
	  victory = 1;
	...
	currentBets.forEach(function(itm) {
		if((roll >= itm.lower) && (roll <= itm.upper)) {
			if(itm.betid == 'undefined') {
				...
				// logger.debug(pool.escape(itm.user));
				// logger.debug(pool.escape(itm.amount));
				setTimeout(function(){
					io.sockets.in(itm.user).emit('message')
				}, 2000);
			}
